...
# Import mermaid tools for parsing-based generation
try:
    ...
    MERMAID_TOOLS_AVAILABLE = True
except ImportError:
    MERMAID_TOOLS_AVAILABLE = False
...
def extract_json_from_output(output_string: str) -> Optional[str]:
    ...
    Returns:
        JSON string or None
    """
    pattern = r"## 3\. Logical Design(.*?)## 4\. Physical Design"
    json_match = re.search(pattern, output_string, re.DOTALL)
    if json_match:
        return json_match.group(1).strip()
    return None


def extract_conceptual_schema_json(output_string: str) -> Optional[Dict[str, Any]]:
    """Extract and parse conceptual schema JSON from agent output.
    
    This function extracts the conceptual design in structured JSON format
    that can be used for Mermaid diagram generation.
    
    Args:
        output_string: Agent output containing conceptual design
        
    Returns:
        Parsed conceptual schema dictionary or None
    """
    if MERMAID_TOOLS_AVAILABLE:
        return extract_conceptual_json(output_string)
    
    # Fallback extraction
    patterns = [
        r'\{[^{}]*["\']Entity Set["\'][^{}]*["\']Relationship Set["\'][^{}]*\}',
        r'```json\s*(\{.*?"Entity Set".*?\})\s*```',
    ]
...
def extract_logical_schema_json(output_string: str) -> Optional[Dict[str, Any]]:
    """Extract and parse logical schema JSON from agent output.
    
    This function extracts the logical design in structured JSON format
    that can be used for DDL generation.
    
    Args:
        output_string: Agent output containing logical design
        
    Returns:
        Parsed logical schema dictionary or None
    """
    if MERMAID_TOOLS_AVAILABLE:
        return extract_logical_json(output_string)
    
    # Fallback extraction
    patterns = [
        r'```json\s*(\{.*?"Attribute".*?"Primary key".*?\})\s*```',
    ]
    ...
    for pattern in patterns:
        ...
        for match in matches:
            try:
                parsed = json.loads(match.replace("'", '"'))
                if 'output' in parsed:
...
# ============== Non-LLM Mermaid Generation ==============

def generate_mermaid_from_schema(schema: Dict[str, Any], schema_type: str = 'conceptual', 
                                  output_dir: Optional[str] = None) -> Tuple[Optional[str], Optional[str]]:
    """Generate Mermaid ER diagram from schema by parsing (not using LLM).
    
    This is faster and more reliable than LLM-based generation.
    
    Args:
        schema: Dictionary containing the schema (conceptual or logical)
        schema_type: Either 'conceptual' or 'logical'
        output_dir: Directory to save the mermaid file
        
    Returns:
        Tuple of (mermaid_code, saved_file_path)
    """
    if not MERMAID_TOOLS_AVAILABLE:
        print("Mermaid tools not available. Please import mermaid_tools module.")
        return None, None
    ...
    try:
        if schema_type == 'conceptual':
            mermaid_code = generate_mermaid_from_conceptual(schema)
        elif schema_type == 'logical':
            mermaid_code = generate_mermaid_from_logical(schema)
        else:
        ...
        if not is_valid:
            print(f"Warning: Generated Mermaid has validation issues: {errors}")
        ...
        file_name = f"{timestamp}_{schema_type}.mmd"
        ...
        print(f"Error generating mermaid from schema: {e}")
        return None, None
