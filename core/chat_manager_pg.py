# -*- coding: utf-8 -*-
"""
PostgreSQL-adapted Chat Manager for multi-agent SQL generation.
This module coordinates the Selector, Decomposer, and Refiner agents.
"""
...
if not logger.handlers:
    ...
    formatter = logging.Formatter('\033[0;35m[%(name)s]\033[0m %(message)s')
...
try:
    ...
    print(f"Use func from core.api in chat_manager_pg.py")
except:
    from core import llm
    LLM_API_FUC = llm.safe_call_llm
    INIT_LOG_PATH_FUNC = llm.init_log_path
    print(f"Use func from core.llm in chat_manager_pg.py")


class ChatManagerPG:
    """
    Chat Manager for PostgreSQL databases.
    Coordinates multi-agent SQL generation pipeline.
    """
    ...
    def __init__(self, 
        ...
        """
        Initialize Chat Manager for PostgreSQL.
        ...
        Args:
            ...
            dataset_name: Dataset name for template selection ('bird', 'spider', 'custom')
            lazy: If True, load schema lazily
            without_selector: If True, skip schema pruning step
        """
...
if __name__ == "__main__":
    # Test with a sample PostgreSQL database
    test_config = {
        'host': 'localhost',
        'port': 5432,
        'user': 'postgres',
    ...
    result = generate_sql_with_agents(
