"""
Domain Knowledge Tools for the Multi-Agent Database Design System.

These tools serve curated, per-domain "ground-truth" rules: standard entity attributes
(required vs. recommended), relationship/cardinality patterns, PostgreSQL datatype mappings,
and normalization guidelines. They are domain-aware and automatically detect whether to use
domain-specific knowledge (e.g., Healthcare, E-Commerce) or fall back to general guidance.

This is complementary to the few-shot retriever in `rag_tools.py` (`get_similar_examples`):
- `get_similar_examples` -> shows worked requirement -> schema examples.
- the tools here -> give explicit design rules for the detected domain.
"""
...
# List of domain-knowledge tools for agent registration
KNOWLEDGE_TOOLS = [
    query_domain_rag,
    get_entity_guidance,
