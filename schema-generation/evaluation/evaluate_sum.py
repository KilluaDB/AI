...
def avg_scores(score_list: list):
    ...
    for i, score in enumerate(score_list):
        if len(score) == 0:
            length -= 1
        for key in score:
            if 'mapping' in key:  # Because some evaluations save schema mapping and attribute mapping
                continue
...
def get_data_sum():
    ...
    for file_name in file_names:
                ...
                try:
                    ...
                    try:
                        ...
                        def replace_single_quotes(match):
                            # Match the answer": {...} part and replace single quotes
                            prefix = match.group(1)  # answer":
                            json_str = match.group(2)  # {...} part
        ...
        print(f' Number of scores present: {len(data_score_list)}')
        avg_score, length = avg_scores(data_score_list)  # Randomly take the first 30
        print(avg_score)
        with open(save_path, 'a+', encoding='utf-8') as f:
            f.write(f'{file_name} length:{length} ' + json.dumps(avg_score) + '\n')

    print('Successfully output to file')
...
def compare_between():
    ...
    ids = list(set(base_direct_datas.keys()) & set(agent_datas.keys()))  # Or use set(list1).intersection(list2)
    # chat
...
def manual_error():
    '''Used to evaluate the accuracy of automated evaluation metrics'''
    ...
    print("Error rate of automated evaluation:")
